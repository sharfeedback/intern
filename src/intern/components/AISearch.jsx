...
const AISearch = () => {
  ...
  const genAI = new GoogleGenerativeAI(
    ...
    "AIzaSyACiotVtegRzCIYJjNqYhfi3w_oPwFI30s"
  );
  ...
  const getResponseForGivenPrompt = async () => {
    ...
    try {
      ...
      const modJson = JSON.stringify(data).replace(/\[(.*?)\]\(.*?\)/g,"$1")
      console.log(modJson)
      const context_search = "Use only the below data to answer the question. Here is the data " + modJson + "and here is the question: " + searchGem + ". Try to include the titles from which you got this answer at the end";
      ...
      setpromptResponses("Something went wrong", error);
      setLoading(false);
  ...
  const renderer = {
    link(href, title, text) {
      if (href.href.startsWith("myapp://")) {
        ...
        return `<a href="" onclick="">${href.text}</a>`;
      }
   ...
   return (
    <div style={{ padding: "20px", minWidth:"80%", wordWrap: "break-word" }}>
     ...
     <Box >
      ...
      <div style={{ wordWrap: "break-word" }} dangerouslySetInnerHTML={{ __html: marked("# Hello") }} />

      {/* <div style={{ wordWrap: "break-word" }} dangerouslySetInnerHTML={{ __html: marked(promptResponses) }} /> */}
